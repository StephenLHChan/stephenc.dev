  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      try {
        ...
        // Choose API endpoint based on subfolder
        const apiUrl = subfolder
          ? `/api/photos/subfolder/${encodeURIComponent(subfolder)}`
          : '/api/photos'
      ...
      <div className="w-full text-center py-8">
        <div className="text-gray-500 dark:text-gray-400 mb-4">
          ...
          <p className="text-lg font-medium">
            {subfolder ? 'No photos in this collection' : 'No photos available'}
          </p>
        </div>
        <p className="text-gray-600 dark:text-gray-400 mb-4">
          {subfolder
            ? 'This photo collection is empty. Check back later for new photos.'
